...
# shellcheck source=../packaging/payload.sh
...
install_wrapper() {
  install_text_file 755 "$BIN_PATH" "$(payload_command_wrapper "$NAME" "${LIB_DIR}/${NAME}")"
}
...
if [ -r \"\${_ssh_multisession_resume_lib}/runtime/profile-entry.sh\" ]; then
  SSH_MULTISESSION_RESUME_LIB=\"\${_ssh_multisession_resume_lib}\"
  export SSH_MULTISESSION_RESUME_LIB
  . \"\${_ssh_multisession_resume_lib}/runtime/profile-entry.sh\"
fi
...
install_files() {
  need_file "${ROOT_DIR}/packaging/payload.sh"
  as_root rm -rf "${LIB_DIR}/client" "${LIB_DIR}/server"
  SSH_MULTISESSION_PAYLOAD_AS_ROOT=as_root payload_install_library "$ROOT_DIR" "$LIB_DIR"
  install_wrapper
  install_profile_hook
  install_profile_bridge
  install_zsh_hooks
